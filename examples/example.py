...
# and that's all! The fdrtd server is now running on http://localhost:5000!
...
# This example assumes that a datashield virtual machine is available at http://192.168.56.100:8080/. For instructions
# on how to get it running, please visit the DataSHIELD wiki (link in README).
...
# with protocol_DataSHIELD, you can print the progress of any function live, just like it is visible in R
# when a function is called, a function_callback is returned to the client while the function keeps running on the
# server in a separate thread.
# while it is running, you can use the following "result" function to get the live progress bar
# it will also return the end result of the function call
...
# The end result of a login function called on the login microservice is a connection callback
# You can use it to start calling functions from the dsBaseClient library
# There are multiple convenient ways of calling these functions.
# You can either pass the arguments to the function directly as kwargs, or you can specify them as a dictionary
# and pass it as the kwarg "parameters". e. g.:
# connection_callback.quantileMean(x='D$LAB_HDL')
# connection_callback.quantileMean(parameters={'x':'D$LAB_HDL'})
# please note that "ds." has been removed from the beginning of the function name.
# Also, the "datasources" kwarg is not
# to be used here. By default, all the servers in the connection are passed on to the "datasources" argument. If
# you need to perform the computation on only a limited number of servers, please include the argument "servers" as
# a list of indices as R would index them (indexing begins at 1 in R and at 0 in Python).
...
# The client should note that in Python, the "." character is reserved whereas in R, it can be freely used in
# variable names. Hence, if passing an argument that contains a "." in the name, please replace it by an "_" or
# use the parameters dictionary instead. Same goes for function names. e. g.:
